...
export function MediaItemTile({ item, width = 150, height = 218 }: MediaItemTileProps) {
  ...
  return (
    ...
    >
      ...
      {/* Progress bar (top) */}
      {progress > 0 ? (
        <ProgressBar hover={overlayVisible} testID="progress-bar">
...
const ProgressBar = styled.View<{ hover?: boolean }>`
  ...
  left: 0;
  right: 0;
  top: ${({ hover }: { hover?: boolean }) => (hover ? '-3px' : '-5px')};
  height: 5px;
...
const ProgressFill = styled.View<{ progress: number }>`
  ...
  width: ${({ progress }: { progress: number }) => `${Math.max(0, Math.min(1, progress)) * 100}%`};
`;
